...
const int safeDistance = 1100; //車速=tP時，看到障礙物到完全煞停的所需距離 用safeDistTest()測,1/100 cm
const int turnSD = safeDistance + 500; //障礙物前煞停+轉90度， 用turnSDTest()測,1/100 cm
const int carVec = 6900;//車子以 tP 前進1秒的距離，1/100cm per second
//PID
const int pivot = 1400;//前進時要走在離牆多遠的線上 單位:1/100 cm, mm*10 cos(30)=0.866 cos(60)=0.5
float kP = 0.2;//0.125;//正比於err造成的輪速改變 => 影響誤差反應幅度。通常是小數
float kD = 1.0;//誤差的微分

const int errMax = int(tP / kP * 1.06); //err上限，影響 tP 可變範圍，越大則 tP 可變範圍越大，則可做出越尖銳的轉彎
const int errDeadzone = 10; // 1/100 cm, 反應死區
//^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
bool isRightWalled()  //
{
  ...
  if (sensorValue[RIGHT] < pivot * 1.1) return true;
  else return false;
}
